...
function ModuleEightContent() {
  ...
  const lessonId = `module8_step${stepForCard}`;
  const lastPath = `/module/module8?step=${activeStep}`;
  const totalSteps = 4;
  ...
  const goToStep = (step: number) => {
    setActiveStep(step);
    router.push(`/module/module8?step=${step}`, { scroll: false });
  };
      ...
      {activeStep === 4 && (
        <section className="min-h-screen flex items-center justify-center px-6">
          <div className="max-w-xl w-full bg-white p-12 rounded-[40px] shadow-xl border border-slate-100 text-center animate-in zoom-in duration-500">
            ...
            <h2 className="text-4xl font-black mb-4 text-slate-900">Module 8 Complete!</h2>
            <p className="text-lg text-[#4F7D96] mb-8 leading-relaxed">
              Congratulations! You've mastered Passive vs Active Investing strategies.
            </p>
            ...
            <div className="mb-8">
              <div className="flex justify-between text-sm mb-2">
                <span className="font-bold text-slate-700">Module 8 Progress</span>

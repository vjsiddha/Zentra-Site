...
async function mockInvestorCall<T>(endpoint: string, username: string, options?: RequestInit): Promise<T> {
  const sep = endpoint.includes('?') ? '&' : '?';
  const url = `${MOCK_INVESTOR_API}${endpoint}${sep}user=${encodeURIComponent(username)}`;
...
const SECTORS: Record<string, { symbols: string[]; icon: string; description: string; color: string }> = {
  ...
  ETFs:          { symbols: ["SPY","QQQ","VTI","VOO"],                           icon: "💼", description: "Diversified funds — safer for beginners.", color: '#10b981' },
  Banking:       { symbols: ["JPM","BAC","WFC","GS"],                            icon: "🏦", description: "Major banks with stable dividends.",       color: '#f59e0b' },
  "Green Energy":{ symbols: ["ENPH","NEE"],                                      icon: "🌱", description: "Clean energy companies.",                  color: '#ef4444' },
  Crypto:        { symbols: ["BTC-USD","ETH-USD"],                               icon: "💎", description: "Digital currencies (high risk).",          color: '#8b5cf6' },
};
...
// ── Reconstruct equity curve from transaction history ────────────────────────
// Walks through trades chronologically, tracking cash + cost-basis of holdings.
// Final point is replaced with live market value for accuracy.
function buildEquityCurveFromHistory(
  history: any[],
  ...
  points.push({
    time:  new Date(sorted[0].ts).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="p-4 rounded-xl bg-slate-50">
          ...
          <p className="text-lg font-bold text-slate-900">${sv.toLocaleString('en-US', { maximumFractionDigits: 0 })}</p>
        </div>
        <div className="p-4 rounded-xl bg-slate-50">
          <p className="text-xs text-slate-500 mb-1">Current Value</p>
          <p className="text-lg font-bold text-slate-900">${totalEquity.toLocaleString('en-US', { maximumFractionDigits: 0 })}</p>
        </div>
        <div className={`p-4 rounded-xl ${pos ? 'bg-emerald-50' : 'bg-red-50'}`}>
          ...
          <p className={`text-lg font-bold ${pos ? 'text-emerald-700' : 'text-red-700'}`}>
            {pos ? '+' : ''}${g.toLocaleString('en-US', { maximumFractionDigits: 0 })} ({pos ? '+' : ''}{gp}%)
          </p>
        </div>
      </div>
      ...
      <div className="h-[280px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={historicalPoints} margin={{ top: 5, right: 5, bottom: 0, left: 0 }}>
            ...
            <XAxis dataKey="time" tick={{ fontSize: 10 }} tickLine={false} axisLine={false} angle={-30} textAnchor="end" height={50} />
            <YAxis domain={['auto', 'auto']} tickFormatter={v => `$${(v / 1000).toFixed(0)}k`} tick={{ fontSize: 10 }} tickLine={false} axisLine={false} />
            <Tooltip
              formatter={(v: any) => [`$${(isFinite(Number(v)) ? Number(v) : 0).toLocaleString('en-US', { maximumFractionDigits: 0 })}`, 'Portfolio Value']}
              labelFormatter={l => `Date: ${l}`}
              contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}
            />
            <Area type="monotone" dataKey="value" stroke="#0B5E8E" fillOpacity={1} fill="url(#histGrad)" strokeWidth={3} dot={{ r: 4, fill: '#0B5E8E', strokeWidth: 0 }} activeDot={{ r: 6 }} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      <p className="text-xs text-slate-400 text-center">
        📊 Equity reconstructed from your {history.length} trade{history.length !== 1 ? 's' : ''} • Each dot = a trade event • Final point uses live prices
      </p>
...
// ── Allocation Pie ───────────────────────────────────────────────────────────
function PortfolioAllocationChart({ positions, marketPrices }: { positions: any; marketPrices: Record<string, number> }) {
  const syms = Object.keys(positions);
  if (!syms.length) return (
    <div className="h-[300px] flex items-center justify-center bg-slate-50 rounded-2xl border border-dashed border-slate-300">
      <div className="text-center"><p className="text-4xl mb-3">🥧</p><p className="font-semibold text-slate-700">No positions yet</p></div>
    </div>
  );
  const sb: Record<string, number> = {};
  syms.forEach(s => {
    const sec   = getSectorForSymbol(s);
    const price = marketPrices[s] ?? positions[s].avg_cost;
    sb[sec] = (sb[sec] || 0) + positions[s].qty * price;
  });
  const cd  = Object.entries(sb).map(([n, v]) => ({ name: n, value: v, color: SECTORS[n]?.color || '#94a3b8' }));
  ...
  return (
    <div className="space-y-4">
      <div className="h-[300px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={cd} cx="50%" cy="50%" labelLine={false} label={({ name, percent }) => `${name} ${(((percent ?? 0) * 100)).toFixed(0)}%`} outerRadius={80} dataKey="value">
              {cd.map((e, i) => <Cell key={i} fill={e.color} />)}
            </Pie>
            <Tooltip formatter={v => `$${Number(v ?? 0).toLocaleString()}`} contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }} />
          </PieChart>
        </ResponsiveContainer>
      </div>
      <div className="space-y-2">
        {cd.map(s => (
          <div key={s.name} className="flex items-center justify-between p-3 rounded-lg bg-slate-50">
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: s.color }} /><span className="font-medium text-slate-700">{s.name}</span></div>
            <div className="text-right"><p className="font-semibold text-slate-900">${s.value.toLocaleString()}</p><p className="text-xs text-slate-500">{((s.value / tot) * 100).toFixed(1)}%</p></div>
          </div>
        ))}
      </div>
    </div>
...
export default function SimulatorPage() {
  ...
  const fetchMarketValue = useCallback(async () => {
    if (!portfolio || !Object.keys(portfolio.positions || {}).length) {
      setCurrentMarketValue(0); setMonthlyDividends(0); setMarketPrices({}); return;
    }
    try {
      ...
      if (mtm.prices) setMarketPrices(mtm.prices);
      let ann = 0;
      for (const sym of Object.keys(portfolio.positions || {})) {
        try {
          ...
          if (d.ok && d.data.dividend_yield) {
            ann += portfolio.positions[sym].qty * (mtm.prices?.[sym] || portfolio.positions[sym].avg_cost) * (d.data.dividend_yield / 100);
          }
        } catch {}
      }
      setMonthlyDividends(ann / 12);
    } catch {
      const fb = Object.values(portfolio.positions || {}).reduce((s: number, p: any) => s + p.qty * p.avg_cost, 0);
      setCurrentMarketValue(fb as number); setMonthlyDividends(0);
    } finally { setMarketValueLoading(false); }
  }, [portfolio, username]);

  const fetchMarketValueRef = useRef(fetchMarketValue);
  useEffect(() => { fetchMarketValueRef.current = fetchMarketValue; }, [fetchMarketValue]);
  useEffect(() => {
    fetchMarketValueRef.current();
    const t = setInterval(() => fetchMarketValueRef.current(), 30000);
    return () => clearInterval(t);
  }, []);
  useEffect(() => { if (portfolio) fetchMarketValueRef.current(); }, [portfolio]);

  const loadPortfolioData = useCallback(async () => {
    if (authLoading) return;
    try {
      setLoading(true);
      const data = await mockInvestorCall<any>('/portfolio', username);
      setPortfolio(data);
      if (data?.starting_cash) setStartingEquity(data.starting_cash);
      setError(null);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      try { setSearchResults(await searchSymbols(searchQuery)); }
  ...
  useEffect(() => {
    ...
    const run = () => getQuote(sellSymbol, username).then(q => setSellPrice(q.last)).catch(() => setSellPrice(null));
    run();
    const t = setInterval(run, 30000);
    return () => clearInterval(t);
  }, [sellSymbol, username]);
  ...
  const handleBuy = async () => {
    if (!selectedSymbol || buyQty <= 0 || !currentPrice) { setTradeError("Please select a stock and enter quantity"); return; }
    setTradeLoading(true); setTradeError(null); setTradeSuccess(null);
    try {
      await mockInvestorCall<any>('/orders/buy', username, { method: 'POST', body: JSON.stringify({ symbol: selectedSymbol, qty: buyQty, price: currentPrice }) });
  ...
  const handleSell = async () => {
    if (!sellSymbol || sellQty <= 0 || !sellPrice) { setTradeError("Please select a position and enter quantity"); return; }
    setTradeLoading(true); setTradeError(null); setTradeSuccess(null);
    try {
      await mockInvestorCall<any>('/orders/sell', username, { method: 'POST', body: JSON.stringify({ symbol: sellSymbol, qty: sellQty, price: sellPrice }) });
      setTradeSuccess(`Sold ${sellQty} shares of ${sellSymbol} at $${sellPrice.toFixed(2)}`);
      setSellQty(0); setSellSymbol("");
      await loadPortfolioData();
    } catch (e) { setTradeError(e instanceof Error ? e.message : 'Trade failed'); }
    finally { setTradeLoading(false); }
  };
  ...
  const handleReset = async () => {
    ...
    try {
      await mockInvestorCall<any>('/portfolio/reset', username, { method: 'POST', body: JSON.stringify({ starting_cash: 100000 }) });
      await loadPortfolioData();
      setTradeSuccess('Portfolio reset to $100,000');
      setCurrentMarketValue(0); setMarketPrices({}); setStartingEquity(100000);
    } catch { setTradeError('Failed to reset portfolio'); }
  };
  ...
  const totalGain       = totalEquity - startingEquity;
  const percentageGain  = (totalGain / startingEquity) * 100;
  ...
  const preTradImpact: PreTradeImpact | null = selectedSymbol && buyQty > 0 && currentPrice
    ? computePreTradeImpact(selectedSymbol, buyQty, currentPrice, positions, cashBalance) : null;
  ...
  if (error) return (
    <main className="min-h-screen flex items-center justify-center" style={{ backgroundColor: COLORS.background }}>
      <div className="text-center max-w-md p-6">
        ...
        <button onClick={loadPortfolioData} className="px-6 py-2 rounded-full text-white font-semibold" style={{ backgroundColor: COLORS.primary }}>Retry</button>
      </div>
    </main>
  );
  ...
  return (
    <main className="min-h-screen" style={{ backgroundColor: COLORS.background }}>
                      ...
                      return (
                        <button key={sym} onClick={() => setSelectedAsset(sym)}
                          ...
                          <div className="text-right">
                            <p className="font-semibold text-slate-900">${liveValue.toFixed(2)}</p>
                            <p className={`text-xs ${gain >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>{gain >= 0 ? '+' : ''}${gain.toFixed(2)}</p>
                          </div>
                        </button>
                      );
                    })}
            ...
            <div className="mb-6">
              ...
              {Object.entries(SECTORS).map(([name, sector]) => (
                <div key={name} className="mb-2">
                  <button onClick={() => setExpandedSector(expandedSector === name ? null : name)} className="w-full flex items-center justify-between p-3 rounded-xl ring-1 hover:bg-slate-50" style={{ borderColor: COLORS.cardBorder }}>
                    ...
                    <div className="mt-2 p-3 rounded-xl text-sm" style={{ backgroundColor: COLORS.primaryLight }}>
                      ...
                      <div className="space-y-1">
                        {sector.symbols.map(sym => (
                          <button key={sym} onClick={() => handleSelectStock(sym)} className="w-full text-left px-3 py-2 rounded-lg bg-white hover:ring-1">
        ...
        <div className="flex-1">
          <header className="bg-white border-b px-6 py-4" style={{ borderColor: COLORS.cardBorder }}>
            <div className="max-w-6xl mx-auto flex items-center justify-between">
              ...
              <div className="flex items-center gap-6">
                <div className="text-right"><p className="text-xs text-slate-500">Cash</p><p className="text-xl font-bold" style={{ color: COLORS.primary }}>${cashBalance.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p></div>
                <div className="text-right"><p className="text-xs text-slate-500">Total Equity</p><p className="text-xl font-bold text-slate-900">{marketValueLoading ? <span className="text-slate-400">Loading...</span> : `$${totalEquity.toLocaleString('en-US', { minimumFractionDigits: 2 })}`}</p></div>
                <div className={`px-3 py-1.5 rounded-full text-sm font-semibold ${isPositive ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>{isPositive ? '+' : ''}{percentageGain.toFixed(2)}%</div>
                <div className="text-right"><p className="text-xs text-slate-500">Total Gain</p><p className={`text-sm font-bold ${isPositive ? 'text-emerald-700' : 'text-red-700'}`}>{isPositive ? '+' : ''}${totalGain.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p></div>
              </div>
            </div>
          </header>
            ...
            <div className="max-w-6xl mx-auto flex gap-1">
              {[
                ...
                const criticalCount = tab.id === 'dss'
                  ? dssInsights.filter(i => i.severity === 'critical' || i.severity === 'warning').length : 0;
                return (
          ...
          <div className="p-6">
            <div className="max-w-6xl mx-auto">
              ...
              {dssNudge && (
                <div className="mb-4 p-4 rounded-xl ring-1 flex justify-between items-start gap-3" style={{ backgroundColor: COLORS.warningLight, borderColor: '#FDE68A' }}>
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-amber-900">{dssNudge}</p>
                    <button onClick={() => { setActiveTab('dss'); setDssNudge(null); }} className="text-xs font-bold mt-1 hover:underline" style={{ color: COLORS.primary }}>View DSS Insights →</button>
                  </div>
                  <button onClick={() => setDssNudge(null)} className="text-slate-400 hover:text-slate-600 flex-shrink-0">✕</button>
                </div>
              )}
              {tradeError && <div className="mb-4 p-4 rounded-xl bg-red-50 text-red-700 ring-1 ring-red-200 flex justify-between"><span>⚠️ {tradeError}</span><button onClick={() => setTradeError(null)}>✕</button></div>}
              {tradeSuccess && <div className="mb-4 p-4 rounded-xl bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200 flex justify-between"><span>✅ {tradeSuccess}</span><button onClick={() => setTradeSuccess(null)}>✕</button></div>}
              ...
              {activeTab === 'trade' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    {!selectedSymbol ? (
                      <div className="space-y-4">
                        <div>
                          ...
                          <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search by symbol or name (e.g. AAPL, Tesla)..." className="w-full px-4 py-3 rounded-xl ring-1 text-sm" style={{ borderColor: COLORS.cardBorder }} />
                          ...
                          {searchResults.length > 0 && (
                            <div className="mt-2 max-h-48 overflow-y-auto rounded-xl ring-1" style={{ borderColor: COLORS.cardBorder }}>
                              {searchResults.map(s => (
                                <button key={s.symbol} onClick={() => handleSelectStock(s.symbol)} className="w-full text-left px-4 py-3 hover:bg-slate-50 border-b last:border-b-0 text-sm" style={{ borderColor: COLORS.cardBorder }}>
                      ...
                      <div className="space-y-4">
                        <div className="p-4 rounded-xl" style={{ backgroundColor: COLORS.primaryLight }}>
                          <div className="flex justify-between items-center">
                            <div><p className="text-sm text-slate-600">Selected Stock</p><p className="text-xl font-bold" style={{ color: COLORS.primary }}>{selectedSymbol}</p></div>
                            <button onClick={() => setSelectedSymbol("")} className="text-slate-400 hover:text-slate-600">✕</button>
                          </div>
                        </div>
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <div className="text-right">
                              {priceLoading ? <span className="text-slate-400">Loading...</span>
                                : currentPrice ? <span className="text-2xl font-bold" style={{ color: COLORS.success }}>${currentPrice.toFixed(2)}</span>
                        ...
                        <div>
                          ...
                          <input type="number" value={buyQty || ''} onChange={e => setBuyQty(parseFloat(e.target.value) || 0)} min="0" step="1" placeholder="Enter number of shares" className="w-full px-4 py-3 rounded-xl ring-1" style={{ borderColor: COLORS.cardBorder }} />
                          {currentPrice && buyQty > 0 && <p className="text-sm text-slate-500 mt-2">Total: <span className="font-semibold text-slate-900">${buyTotal.toFixed(2)}</span>{buyTotal > cashBalance && <span className="text-red-500 ml-2">(Insufficient funds)</span>}</p>}
                        </div>
                        <PreTradeWarning impact={preTradImpact} qty={buyQty} price={currentPrice} />
                        <button onClick={handleBuy} disabled={tradeLoading || !canBuy} className="w-full py-3 rounded-full text-white font-semibold disabled:opacity-50" style={{ backgroundColor: COLORS.success }}>
                  ...
                  <Card title="Sell Stocks" icon="📉" iconBg={COLORS.dangerLight} iconColor={COLORS.danger}>
                    {positionSymbols.length === 0 ? (
                      <div className="text-center py-8"><p className="text-4xl mb-3">📭</p><p className="font-semibold text-slate-700">No positions</p><p className="text-sm text-slate-500 mt-1">Buy some stocks first!</p></div>
                    ) : !sellSymbol ? (
                      <div className="space-y-4">
                        <div className="p-4 rounded-xl" style={{ backgroundColor: COLORS.primaryLight }}>
                          <p className="text-sm text-slate-600 mb-3">Select Position</p>
                          <select value={sellSymbol} onChange={e => { setSellSymbol(e.target.value); setSellQty(0); }} className="w-full px-4 py-2.5 rounded-lg bg-white ring-1 font-medium cursor-pointer" style={{ borderColor: COLORS.cardBorder, color: COLORS.primary }}>
                            <option value="">Select a position...</option>
                            {positionSymbols.map(sym => <option key={sym} value={sym}>{sym} — {positions[sym].qty} shares @ ${positions[sym].avg_cost.toFixed(2)} avg</option>)}
                          </select>
                        </div>
                        <div className="p-4 rounded-xl bg-slate-50 border border-dashed" style={{ borderColor: COLORS.cardBorder }}><p className="text-center text-slate-500 text-sm">Choose a position above to start selling</p></div>
                      </div>
                    ) : (
                      <div className="space-y-4">
                        <div className="p-4 rounded-xl" style={{ backgroundColor: COLORS.primaryLight }}>
                          <div className="flex justify-between items-center">
                            <div><p className="text-sm text-slate-600">Selected Position</p><p className="text-xl font-bold" style={{ color: COLORS.primary }}>{sellSymbol}</p><p className="text-xs text-slate-500 mt-1">You own {maxSellQty} shares @ ${positions[sellSymbol].avg_cost.toFixed(2)} avg</p></div>
                            <button onClick={() => { setSellSymbol(""); setSellQty(0); }} className="text-slate-400 hover:text-slate-600">✕</button>
                          </div>
                        </div>
                        <div className="p-4 rounded-xl" style={{ backgroundColor: COLORS.dangerLight }}>
                          <div className="flex justify-between"><span className="text-slate-600">Current Price</span><span className="text-2xl font-bold" style={{ color: COLORS.danger }}>{sellPrice ? `$${sellPrice.toFixed(2)}` : 'Loading...'}</span></div>
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-slate-700 mb-2">Quantity to Sell</label>
                          <input type="number" value={sellQty || ''} onChange={e => setSellQty(parseFloat(e.target.value) || 0)} min="0" max={maxSellQty} step="1" placeholder="Enter number of shares" className="w-full px-4 py-3 rounded-xl ring-1" style={{ borderColor: COLORS.cardBorder }} />
                          <div className="flex justify-between items-center mt-2">
                            <button onClick={() => setSellQty(maxSellQty)} className="text-xs font-medium hover:underline" style={{ color: COLORS.primary }}>Sell all ({maxSellQty})</button>
                            {sellPrice && sellQty > 0 && <p className="text-sm text-slate-500">Total: <span className="font-semibold text-slate-900">${(sellQty * sellPrice).toFixed(2)}</span></p>}
                          </div>
                        </div>
                        <button onClick={handleSell} disabled={tradeLoading || !canSell} className="w-full py-3 rounded-full text-white font-semibold disabled:opacity-50" style={{ backgroundColor: COLORS.danger }}>
                  ...
                  <div className="lg:col-span-2">
                    <Card>
                      <div className="flex justify-between mb-3"><span className="font-semibold text-slate-900">💵 Available Cash</span><span className="font-bold" style={{ color: COLORS.primary }}>${cashBalance.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span></div>
                      <div className="w-full h-3 rounded-full bg-slate-100"><div className="h-full rounded-full" style={{ width: `${Math.min((cashBalance / startingEquity) * 100, 100)}%`, backgroundColor: COLORS.primary }} /></div>
                      <div className="flex justify-between mt-2"><p className="text-sm text-slate-500">{((cashBalance / startingEquity) * 100).toFixed(1)}% remaining</p><button onClick={handleReset} className="text-xs text-slate-400 hover:text-red-500">Reset Portfolio</button></div>
                    </Card>
                  </div>
                  <div className="lg:col-span-2 p-4 rounded-2xl" style={{ backgroundColor: COLORS.warningLight }}><p className="text-slate-700">{currentTip}</p></div>
                </div>
              )}
              ...
              {activeTab === 'portfolio' && (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <Card title="Portfolio Allocation" icon="🥧"><PortfolioAllocationChart positions={positions} marketPrices={marketPrices} /></Card>
                    <div className="space-y-6">
                      <Card title="Your Positions" icon="📦">
                        {positionSymbols.length === 0 ? (
                          <div className="text-center py-8"><p className="text-4xl mb-3">📭</p><p className="font-semibold text-slate-700">No positions yet</p><p className="text-sm text-slate-500 mt-1">Start trading!</p></div>
                        ) : (
                          <div className="space-y-2">
                            {positionSymbols.map(sym => {
                              ...
                              return (
                                <div key={sym} className="flex justify-between items-center p-3 rounded-lg bg-slate-50">
                                  <div>
                                    <button onClick={() => setSelectedAsset(sym)} className="font-semibold hover:underline cursor-pointer" style={{ color: COLORS.primary }}>{sym}</button>
                                    <p className="text-xs text-slate-500">{pos.qty} shares @ ${pos.avg_cost.toFixed(2)} avg</p>
                                  </div>
                                  <div className="text-right">
                                    <p className="font-bold text-slate-900">${liveValue.toFixed(2)}</p>
                                    <p className={`text-xs ${gain >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>{gain >= 0 ? '+' : ''}${gain.toFixed(2)}</p>
                                  </div>
                                </div>
                              );
                      ...
                      <Card title="Portfolio Analysis" icon="🧠">
                        <div className="flex items-center gap-4 p-4 rounded-xl ring-1 cursor-pointer hover:opacity-90 transition-opacity"
                          ...
                          <div className="flex-1">
                            <p className="font-bold text-slate-800">{dssScorecard.overallScore}/100 overall score</p>
                            <div className="flex items-center gap-2 mt-1">
                              <div className="w-2 h-2 rounded-full" style={{ backgroundColor: riskColor }} />
                              <p className="text-xs text-slate-600">{riskLabel} · {dssScorecard.components.riskBalance.note}</p>
                            </div>
                            <p className="text-xs text-slate-500 mt-1">{dssScorecard.topDrag}</p>
                            <p className="text-xs font-semibold mt-2" style={{ color: COLORS.primary }}>View full analysis →</p>
                          </div>
                        </div>
                      </Card>
                      ...
                      <Card title="Estimated Monthly Dividends" icon="💰">
                        <div className="p-4 rounded-xl" style={{ backgroundColor: COLORS.primaryLight }}>
                          ...
                          <p className="text-2xl font-bold" style={{ color: COLORS.primary }}>${monthlyDividends.toFixed(2)}</p>
                          <p className="text-xs text-slate-500 mt-2">{monthlyDividends > 0 ? `$${(monthlyDividends * 12).toFixed(2)} annually from dividend-paying stocks` : 'None of your holdings pay dividends'}</p>
                        </div>
                      </Card>
                    </div>
                  </div>
                ...
                <div className="space-y-6">
                  <Card title="Transaction History" icon="📋">
                    {!portfolio?.history || portfolio.history.length === 0 ? (
                      <div className="text-center py-12"><p className="text-5xl mb-4">📭</p><p className="font-semibold text-slate-700">No transactions yet</p><p className="text-sm text-slate-500 mt-1">Start trading to see your history!</p></div>
                    ) : (
                        ...
                        <table className="w-full">
                          <thead>
                            <tr className="text-left text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b" style={{ borderColor: COLORS.cardBorder }}>
                              <th className="pb-4">Date</th><th className="pb-4">Side</th><th className="pb-4">Symbol</th><th className="pb-4 text-center">Qty</th><th className="pb-4 text-right">Total Amount</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-slate-50">
                            {[...portfolio.history].reverse().map((txn: any, idx: number) => {
                              const side = txn.type || txn.side; const isBuy = side === 'BUY';
                              return (
                                <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                  <td className="py-4 text-xs text-slate-500 font-medium">{new Date(txn.ts).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}</td>
                                  <td className="py-4"><span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${isBuy ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>{side}</span></td>
                                  <td className="py-4"><button onClick={() => setSelectedAsset(txn.ticker)} className="font-black hover:underline cursor-pointer" style={{ color: COLORS.primary }}>{txn.ticker || 'CASH'}</button></td>
                                  <td className={`py-4 text-center font-bold ${isBuy ? 'text-slate-700' : 'text-rose-600'}`}>{isBuy ? `+${txn.qty}` : `-${txn.qty}`}</td>
                                  <td className="py-4 text-right font-black text-slate-900">${(txn.qty * txn.price).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                </tr>
                              );
                            })}
                          </tbody>
              ...
              {activeTab === 'dss' && (
                <div className="space-y-6">
                  <DSSInsightsPanel insights={dssInsights} rebalanceFlags={dssRebalanceFlags} scorecard={dssScorecard} hasPositions={positionSymbols.length > 0} />
                </div>
              )}
            </div>
          </div>
      ...
      {selectedAsset && (
        <AssetDashboard symbol={selectedAsset} portfolio={portfolio} username={username} onClose={() => setSelectedAsset(null)} />
      )}
    </main>
  );
}
    ...
    <div className="bg-white rounded-2xl p-6 ring-1" style={{ borderColor: COLORS.cardBorder }}>
      {title && (
        <div className="flex items-center gap-3 mb-5">
          {icon && <div className="flex h-9 w-9 items-center justify-center rounded-xl text-sm" style={{ backgroundColor: iconBg, color: iconColor }}>{icon}</div>}

...
function PortfolioValueChart({ portfolio, username }: { portfolio: any; username: string }) {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        const data = await mockInvestorCall<any[]>(`/metrics/equity-curve?period=${p}&interval=1d`, username);
        if (!data || data.length === 0) {
          setChartData([{ time: new Date().toLocaleDateString(), value: portfolio?.cash || 100000 }]);
        } else {
          let fd = data;
          if (period !== "ALL") {
            const cut = new Date();
          ...
          setChartData(fd.map((x: any) => ({
            time:  new Date(x.ts).toLocaleDateString('en-US', { month:'short', day:'numeric' }),
            value: x.equity,
          })));
        }
      } catch {
  ...
  if (loading) return (
    <div className="h-[300px] flex items-center justify-center bg-slate-50 rounded-2xl">
      <div className="text-center">
        <div className="animate-spin w-8 h-8 border-4 border-t-transparent rounded-full mx-auto mb-2" style={{ borderColor: COLORS.primary, borderTopColor:'transparent' }} />
  ...
  if (!chartData.length) return (
    <div className="h-[300px] flex items-center justify-center bg-slate-50 rounded-2xl border border-dashed border-slate-300">
      <div className="text-center"><p className="text-4xl mb-3">📊</p><p className="font-semibold text-slate-700">No data yet</p></div>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="p-4 rounded-xl bg-slate-50"><p className="text-xs text-slate-500 mb-1">Starting Value</p><p className="text-lg font-bold text-slate-900">${sv.toLocaleString()}</p></div>
        <div className="p-4 rounded-xl bg-slate-50"><p className="text-xs text-slate-500 mb-1">Current Value</p><p className="text-lg font-bold text-slate-900">${cv.toLocaleString()}</p></div>
        <div className={`p-4 rounded-xl ${pos?'bg-emerald-50':'bg-red-50'}`}>
          <p className="text-xs text-slate-500 mb-1">Total Gain</p>
          <p className={`text-lg font-bold ${pos?'text-emerald-700':'text-red-700'}`}>{pos?'+':''} ${g.toLocaleString()} ({pos?'+':''}{gp}%)</p>
        </div>
      </div>
      <div className="h-[300px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData}>
            ...
            <XAxis dataKey="time" tick={{fontSize:10}} tickLine={false} axisLine={false} angle={-45} textAnchor="end" height={60}/>
            <YAxis domain={['auto','auto']} tickFormatter={v=>`$${(v/1000).toFixed(0)}k`} tick={{fontSize:10}} tickLine={false} axisLine={false}/>
            <Tooltip formatter={(v:any)=>[`$${(isFinite(Number(v))?Number(v):0).toLocaleString()}`,"Portfolio Value"]} labelFormatter={l=>`Date: ${l}`} contentStyle={{borderRadius:'12px',border:'none',boxShadow:'0 4px 12px rgba(0,0,0,0.1)'}}/>
            <Area type="monotone" dataKey="value" stroke="#0B5E8E" fillOpacity={1} fill="url(#cv)" strokeWidth={3}/>
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="flex gap-2">
        {[{y:1,l:'1 Year'},{y:5,l:'5 Years'},{y:10,l:'10 Years'},{y:20,l:'20 Years'}].map(o=>(
          <button key={o.y} onClick={()=>setTh(o.y as any)}
            className={`px-4 py-2 rounded-lg font-medium transition-colors ${th===o.y?'text-white':'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
...
function PortfolioAllocationChart({ positions }: { positions: any }) {
  ...
  syms.forEach(s => { const sec=getSectorForSymbol(s); sb[sec]=(sb[sec]||0)+positions[s].qty*positions[s].avg_cost; });
...
const [dssNudge,      setDssNudge]      = useState<string | null>(null);  
  ...
  useEffect(() => { fetchMarketValue(); const t=setInterval(fetchMarketValue,30000); return ()=>clearInterval(t); }, [fetchMarketValue]);

  const loadPortfolioData = useCallback(async () => {
    if (authLoading) return;
    try { setLoading(true); setPortfolio(await mockInvestorCall<any>('/portfolio',username)); setError(null); }
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async()=>{ setSearchLoading(true); try{setSearchResults(await searchSymbols(searchQuery));}catch{setSearchResults([]);} finally{setSearchLoading(false);} },300);
  ...
  const handleSell = async () => {
    ...
    try {
      ...
      setSellQty(0);setSellSymbol(""); await loadPortfolioData(); setTimeout(fetchMarketValue,500);
    } catch(e){setTradeError(e instanceof Error?e.message:'Trade failed');}
    finally{setTradeLoading(false);}
  };
  ...
  const handleReset = async () => {
    ...
    try { await mockInvestorCall<any>('/portfolio/reset',username,{method:'POST',body:JSON.stringify({starting_cash:100000})}); await loadPortfolioData(); setTradeSuccess('Portfolio reset to $100,000'); setCurrentMarketValue(0); }
  ...
  return (
    <main className="min-h-screen" style={{backgroundColor:COLORS.background}}>
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="lg:col-span-2">
                    <Card>
                      ...
                      <div className="w-full h-3 rounded-full bg-slate-100"><div className="h-full rounded-full" style={{width:`${Math.min((cashBalance/100000)*100,100)}%`,backgroundColor:COLORS.primary}}/></div>
                      <div className="flex justify-between mt-2"><p className="text-sm text-slate-500">{((cashBalance/100000)*100).toFixed(1)}% remaining</p><button onClick={handleReset} className="text-xs text-slate-400 hover:text-red-500">Reset Portfolio</button></div>
                    </Card>
                  </div>
              ...
              {activeTab==='portfolio'&&(
                <div className="space-y-6">
                  <Card title="Portfolio Value Over Time" icon="📈"><PortfolioValueChart portfolio={portfolio} username={username}/></Card>
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <Card title="Portfolio Allocation" icon="🥧"><PortfolioAllocationChart positions={positions}/></Card>
                    <div className="space-y-6">
                            ...
                            {positionSymbols.map(sym=>{
                              ...
                              return (
                                <div key={sym} className="flex justify-between items-center p-3 rounded-lg bg-slate-50">
                                  <div><button onClick={()=>setSelectedAsset(sym)} className="font-semibold hover:underline cursor-pointer" style={{color:COLORS.primary}}>{sym}</button><p className="text-xs text-slate-500">{pos.qty} shares @ ${pos.avg_cost.toFixed(2)}</p></div>
                                  <p className="font-bold text-slate-900">${(pos.qty*pos.avg_cost).toFixed(2)}</p>
                                </div>
                              );
                            })}
                          </div>
      ...
      {selectedAsset&&<AssetDashboard symbol={selectedAsset} portfolio={portfolio} onClose={()=>setSelectedAsset(null)}/>}
    </main>
  );
}
